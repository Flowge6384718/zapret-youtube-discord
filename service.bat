...
:: Wait for required file in %TEMP% (up to 180 sec)
:: IMPORTANT: replace the filename below with the real file name that should appear
set "REQUIRED_TEMP_FILE=%TEMP%\zapret_prepared.ok"

:: here download call powershell script.
...
:: Usage: call :check_temp_required_file "C:\Path\file.ext"
...
:wait_for_temp_file
:: Usage: call :wait_for_temp_file "C:\Path\file.ext" 180
:: Returns: 0 = file found, 1 = timeout

setlocal EnableDelayedExpansion
set "WAIT_FILE=%~1"
set /a WAIT_SECONDS=%~2

if not defined WAIT_FILE (
    endlocal & exit /b 1
)
if not defined WAIT_SECONDS set /a WAIT_SECONDS=180

:: If file already exists - continue immediately
if exist "!WAIT_FILE!" (
    echo.
    call :PrintGreen "Required files have been prepared."
    endlocal & exit /b 0
)
...
set /a ELAPSED+=1
set /a PERCENT=(ELAPSED*100)/WAIT_SECONDS

:: filled = percent of BAR_LEN (0..30)
set /a FILLED=(PERCENT*BAR_LEN)/100
if !FILLED! gtr !BAR_LEN! set /a FILLED=BAR_LEN
...
if !ELAPSED! lss !WAIT_SECONDS! goto wait_for_temp_file_loop
